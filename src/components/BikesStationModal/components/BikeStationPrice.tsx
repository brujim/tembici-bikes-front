...
export const BikeStationPrice = ({ type, mech, electric, tariff }: PriceProps) => {
  ...
  return (
      ...
      <div className="flex flex-row justify-between items-center bg-pearlgray p-4 mb-6 rounded-2xl">
        ...
        <Image
          ...
          height={24}
        />
        <p className='font-normal text-[14px] leading-4 ml-2'>{serializeTitle()}</p>

        </div>
        {calculateTariff() === 'high' && (
					<div className='w-[30%]'>
						<p className='font-semibold	text-ruby text-[24px]'>{getPriceCurrency(getPrice())}</p>
					</div>
				)}
        {calculateTariff() === 'median' && (
					<div className='w-[30%]'>
						<p className='font-semibold	text-gold text-[24px]'>{getPriceCurrency(getPrice())}</p>
					</div>
				)}
				{calculateTariff() === 'low' && (
					<div className='w-[30%]'>
						<p className='font-semibold	text-sapphire text-[24px]'>{getPriceCurrency(getPrice())}</p>
					</div>
				)}
				{calculateTariff() === 'none' && (
					<div className='w-[30%]'>
						<p className='font-semibold	text-emerald text-[24px]'>{getPriceCurrency(getPrice())}</p>
					</div>
				)}
